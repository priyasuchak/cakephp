...
<div class="large-10 medium-9 columns">	             
    <?= $this->Form->create('updateworkinfo') ?>
    <fieldset>    	        
        <legend><?= __('Update Work Information') ?></legend>
        <?= $this->Form->input('company', ['type' => 'text','label'=>'Company','value'=>h($results->Individual_Company)]); ?>
        <?= $this->Form->input('title', ['type' => 'text','label'=>'Title','value'=>h($results->Individual_Title)]); ?>
        <?= $this->Form->input('department',array('type'=>'select','empty'=>h($deptempty),'label'=>'Department','value'=>h($results->Individual_Department),'options'=>['Accounting'=>'Accounting','Comp and Benefits'=>'Comp and Benefits','Finance'=>'Finance','Human Resources'=>'Human Resources','Legal'=>'Legal','Payroll'=>'Payroll','Shareholder Services'=>'Shareholder Services','Stock Transfer'=>'Stock Transfer','Tax'=>'Tax'])); ?>        
        
        <?= $this->Form->input('workaddress1', ['type' => 'text','label'=>'Work Address Line 1','value'=>h($results->Work_Address1)]); ?>
        <?= $this->Form->input('workaddress2', ['type' => 'text','label'=>'Work Address Line 2','value'=>h($results->Work_Address2)]); ?>
        <?= $this->Form->input('workcity', ['type' => 'text','label'=>'Work City','value'=>h($results->Work_City)]); ?>
             <?= $this->Form->input('workstate', array('type' => 'select','empty'=>h($stateempty),'label'=>'Work State','value'=>h($results->Work_State),'options'=>['Non US'=>'Non US',
'Alabama'=>'Alabama',
...
'Nevada'=>'Nevada','New Hampshire'=>'New Hampshire','New Jersey'=>'New Jersey'])); ?>
        <?= $this->Form->input('workpostcode', ['type' => 'text','label'=>'Work Zip Code','value'=>h($results->Work_Postal_Code)]); ?>
		
        <?= $this->Form->input('workcountry', array('type' => 'select','empty'=>h($countryempty),'label'=>'Work Country','value'=>h($results->Work_Country),'options'=>[   
...
'Yemen'=>'Yemen','Zambia'=>'Zambia','Zimbabwe'=>'Zimbabwe','Other'=>'Other'
        ...
        <?= $this->Form->input('workphone', ['type' => 'tel','label'=>'Work Phone','value'=>h($results->Work_Phone)]); ?>
