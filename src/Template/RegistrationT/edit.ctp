<div class="actions columns large-2 medium-3">
    ...
    <ul class="side-nav">
        <li><?= $this->Form->postLink(
                __('Delete'),
                ['action' => 'delete', $registrationT->Registration_ID],
                ['confirm' => __('Are you sure you want to delete # {0}?', $registrationT->Registration_ID)]
            )
        ?></li>
        <li><?= $this->Html->link(__('List Registration T'), ['action' => 'index']) ?></li>
    </ul>
</div>
<div class="registrationT form large-10 medium-9 columns">
    <?= $this->Form->create($registrationT); ?>
    <fieldset>
        <legend><?= __('Edit Registration T') ?></legend>
        <?php
